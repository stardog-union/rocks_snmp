...
public:
    ...
    enum SnmpValEdge_e
    {
        SNMP_EDGE_REQUEST_DATA=1021, //!< please update my data, someone wants good
        SNMP_EDGE_DATA_GOOD=1022,    //!< data update arrived
        SNMP_EDGE_DATA_STALE=1023,   //!< outside source telling us this is old data,
...
public:
    ...
    void InsertTablePrefix(const OidVector_t & OidAgentPrefix, const OidVector_t & OidTablePrefix,
                           const OidVector_t & OidAreaPrefix, const OidVector_t & OidRowIdSuffix);
...
/** NOTE: this is a template for generic pointers:  neither used nor a derived base. **/
...
public:
    ...
    /// debug support, convert value to string for output
    virtual std::string & GetValueAsString(std::string &Output) {return(Output);};
