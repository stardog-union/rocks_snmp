...
public:
    ...
    virtual void AppendToIovec(const PduSubId & ValId, std::vector<struct iovec> & IoArray) const;

    /// debug support, convert value to string for output
    virtual std::string & GetValueAsString(std::string &Output)
    {char buf[33]; snprintf(buf,33,"%u", (unsigned)m_ValType.m_Type); Output=buf; return(Output);};
...
private:
    ...
    SnmpValError & operator=(const SnmpValError &);  //!< disabled:  assignment operator
