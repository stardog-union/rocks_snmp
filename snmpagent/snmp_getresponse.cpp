 ...
 *
 * @brief Implemenation of snmp agentX response pdu object for Get & GetNext (rfc 2741, January 2000)
 */
    ...
    : ResponsePDU(Agent, Request), m_PendingData(0)
{
    ...
    if (eNoAgentXError==m_Response.m_Error)
    {
        ...
        while(ptr<limit)
        {
            ...
            id_end=(const PduSubId *)(ptr + (4*id_start->m_SubIdLen + sizeof(PduSubId)));
...
GetResponsePDU::Dump()
{
    ...
    for (loop=0; loop<m_ResponsePDUVec.size(); ++loop)
    {
        printf("    m_ResponsePDUVec[%zd].iov_base: %p\n",
               loop, m_ResponsePDUVec[loop].iov_base);
        printf("    m_ResponsePDUVec[%zd].iov_len: %zd\n",
               loop, m_ResponsePDUVec[loop].iov_len);
    }   // for
    ...
    printf("         m_PacketID: %u (%u)\n",
           m_Header.m_PacketID.u, (unsigned)m_Header.m_PacketID.c[3]);
