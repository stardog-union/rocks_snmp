...
public:
    ...
    virtual void TimerCallback()
    {
        ...
        for (count=0, it=m_MgrPtr->tbegin();
             m_MgrPtr->tend()!=it; ++it, ++count);
...
public:
    ...
    objSimplePipeOut(int Handle, int MilliSec) 
        : m_IsGood(true), m_Sent(0)
    {
    ...
    virtual bool WriteAvailCallback()
    {
        ...
        {
            Logging(LOG_ERR, "%s:  writev error (errno=%d)",
                    __func__, errno);
...
public:
    ...
    objSimplePipeIn(int Handle, int MilliSec) 
        : m_IsGood(true), m_Received(0)
    {
        SetFileHandle(Handle);
        SetIntervalMS(MilliSec);
    }   // objSimplePipe
    ...
    virtual bool ReadAvailCallback()
    {
        ...
        {
            Logging(LOG_ERR, "%s:  readv error (errno=%d)",
                    __func__, errno);
