...
bool
ReaderWriter::CloseCallback(int)
{
    Close();
...
ReaderWriter::ReadAvailCallback()
{
    ...
    if (NULL!=m_ReadBuf.get())
    {
        ...
        do
        {
            ...
            ret_val=readv(m_Handle,
                          m_ReadBuf->ReadIovec(), m_ReadBuf->ReadIovecCnt());

            if (0 <= ret_val)
            {
                m_ReadBuf->ReadMarkLen(ret_val);
                // loop if have not reached minimum
                again=((m_ReadBuf->ReadLen() < m_ReadBuf->ReadMinimum())
                       && 0!=ret_val);
                zero_size=(0==ret_val);
            }   // if
            ...
            else
            {
                ...
                else
                {
                    ...
                    ret_flag=false;
                    m_ReadBuf=NULL;
                    Logging(LOG_ERR, "%s: error on readv [errno %d, handle %d]",
        ...
        {
            bool read_flag;
            read_flag=RequestRead(ret_flag && m_ReadBuf->ReadLen() < m_ReadBuf->ReadMinimum());
            if (!read_flag)
            {
                Logging(LOG_ERR, "%s: RequestRead failed (ret_flag=%d, ReadLen=%zd, ReadMinimum=%zd)",
                        __func__, ret_flag, m_ReadBuf->ReadLen(), m_ReadBuf->ReadMinimum());
            }   // if
        }   // if

        // Send a notification?
        if (ret_flag
            && (m_ReadBuf->ReadMinimum() <= m_ReadBuf->ReadLen()
                || (zero_size)))
        {
            m_ReadBuf=NULL;
 ...
 * @date Created 05/13/11
 * @author matthewv
 * @returns true if simultaneous post-Write callbacks should proceed (none today)
 */
bool
ReaderWriter::WriteAvailCallback()
{
    ...
    if (NULL!=m_WriteBuf.get())
    {
        ...
        {
            again=false;
            ret_val=writev(m_Handle,
                          m_WriteBuf->WriteIovec(), m_WriteBuf->WriteIovecCnt());
            ...
            if (0 <= ret_val)
            {
                ...
                if (m_WriteBuf->WriteLen()==m_WriteBuf->WriteEnd()
                    && 0!=m_PendingWrite.size() && m_PendingWrite.front()->IsDataReady())
                {
                ...
                // loop if have not reached minimum
                again=((m_WriteBuf->WriteLen() < m_WriteBuf->WriteEnd())
                       && 0!=ret_val);
...
void
ReaderWriter::Read(
    ReaderWriterBufPtr & Buffer)  //!< buffer to receive data
{
    m_ReadBuf=Buffer;
    ReadAvailCallback();
...
void
ReaderWriter::Write(
    ReaderWriterBufPtr & Buffer)  //!< buffer to transmit
{
    // immediate write?
