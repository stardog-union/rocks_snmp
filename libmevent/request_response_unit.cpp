...
public:
  ...
  bool EdgeNotification(
      unsigned int
          EdgeId, //!< what just happened, what graph edge are we walking
 ...
 * @date 06/08/11  matthewv  Created
 */
void RequestResponseTester::testMemcache() {
  MEventMgr mgr;
 ...
 * @date 06/08/11  matthewv  Created
 */
void RequestResponseTester::testHandlersocket() {
  MEventMgr mgr;
  // galaxy 207.154.57.30
  RequestResponse rr(0xcf9a391e, 9998);
  const char *test_msgs[] = {
      "P\t1\tstm_node1__user_node\tsubscriptions\tPRIMARY\tuser_id,content_id,"
      "parent_content_id\n",
      "1\t>\t3\t0\t0\t0\n", "1\t>\t3\t1\t2473393\t0\n",
