...
protected:
    ...
    std::chrono::steady_clock::time_point m_LastTimeout;            //!< set if previous event was timeout,
...
public:
    ...
    void Init() {m_Handle=-1; Reset();};

    MEventPtr GetMEventPtr() {return std::static_pointer_cast<MEventObj>(GetStateMachinePtr());}
    ...
    void SetIntervalMS(int TimeMS) {m_Interval=std::chrono::milliseconds(TimeMS);};

    /// retrieve default interval for read/write timeout in milliseconds
    int GetIntervalMS() const {return(m_Interval.count());};
    std::chrono::steady_clock::duration GetInterval() const {return m_Interval;};

    void SetNextTimeout(std::chrono::steady_clock::time_point t_point) {m_NextTimeout = t_point;}
    std::chrono::steady_clock::time_point GetNextTimeout() const {return m_NextTimeout;};
    ...
    {std::chrono::milliseconds ms(Millisec); return(SetTimer(ms));};
    ...
    void SetLastTimeout(const std::chrono::steady_clock::time_point & Now) {m_LastTimeout=Now;};
