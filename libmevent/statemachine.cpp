 ...
 */
StateMachine::StateMachine() : m_PrevNode(0), m_CurNode(0) {
  return;
 ...
 * @author matthewv
 */
void StateMachine::AddCompletion(
 ...
 * @author matthewv
 */
void StateMachine::RemoveCompletion(
 ...
 * @author matthewv
 */
void StateMachine::SendCompletion(
    unsigned Event) //!< completion event to send to all watchers
{
  StateMachineSet_t::iterator it;
  StateMachine *ptr;

  for (it = m_CompletionList.begin(); m_CompletionList.end() != it;
       /* ++it below */) {
