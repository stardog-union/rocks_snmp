...
TcpEventSocket::TcpEventSocket()
    : m_NetIp(0), m_NetPort(0), m_NoLinger(false)
{
    ...
    unsigned short PortNetOrder)   //!< network ordered port address as short
{
    ...
    {
        good=false;
        Logging(LOG_ERR, "%s: No assigned event manager.",
                __func__);
    }   // else
...
TcpEventSocket::Connect()
{
    ...
    if (good)
    {
        ...
        {
            // error
            Logging(LOG_ERR, "%s:  connect() failed [err_num=%d, fd=%d].",
                    __func__, err_num, m_Handle);
    ...
    int & Errno)              //!< [output] errno after connect()
{
    ...
    if (0!=m_NetIp && 0!=m_NetPort)
    {
        ...
        {
            Logging(LOG_ERR, "%s: socket() failed with %d, m_NetIp 0x%x, m_NetPort %d",
    ...
    {
        Logging(LOG_ERR, "%s: zero ip and/or port [ip=%u, port=%hu]",
                __func__, m_NetIp, m_NetPort);
        SendEdge(TS_EDGE_ERROR);
        ErrorCallback();
    }   // else
...
TcpEventSocket::Close()
{
    if (-1!=m_Handle)
    {
        ...
        if (m_NoLinger)
        {
            ...
            ret_val=setsockopt(m_Handle, SOL_SOCKET, SO_LINGER, (const char *) &linger,
                       sizeof(linger));
        ...
        if (0!=ret_val)
        {
            Logging(LOG_ERR, "%s: Error with shutdown() [errno=%d]",
                        __func__, errno);
        }   // if
    ...
    bool Set)
{
    ...
    if (-1!=ret_val)
    {
        ...
        if (-1==ret_val)
        {
            Logging(LOG_ERR, "%s: F_SETFL failed [errno=%d, fd=%d].",
    ...
    {
        Logging(LOG_ERR, "%s: F_GETFL failed [errno=%d, fd=%d].",
                __func__, errno, m_Handle);
        ret_flag=false;
    }   // else
