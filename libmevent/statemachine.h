...
typedef std::multiset<StateMachinePtr, StateMachinePtrCompare>
    StateMachineSet_t;
...
protected:
  ...
  // ? std::vector<StateMachinePtr> for watchers on any state change ... ref
  // ptr?
  StateMachineSet_t
      m_CompletionList; //!< for completion watchers
...
public:
  ...
  virtual bool EdgeNotification(unsigned int EdgeId, StateMachinePtr &Caller,
                                bool PreNotify) {
    return (false);
  };
  ...
  bool operator<(const StateMachine &rhs) const { return (this < &rhs); };
...
private:
  ...
  StateMachine &
  operator=(const StateMachine &); //!< disabled:  assignment operator
